        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId === null) {
                    ...
                    throw new DisplayException('Akses ditolak: informasi pemilik server tidak tersedia © TuanVinn Guard.');
                }

                if ($ownerId !== $user->id) {
                    throw new DisplayException('❌Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri © TuanVinn Guard');
                }
            }
