...
class IndexController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, 'Akses ditolak © TuanVinn Guard');
        }
    ...
    public function update(BaseSettingsFormRequest $request): RedirectResponse
    {
        ...
        $this->alert->success(
            'Panel settings have been updated successfully and the queue worker was restarted to apply these changes © TuanVinn Guard.'
        )->flash();
